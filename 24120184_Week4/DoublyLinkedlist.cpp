...
d_List *createList(d_NODE *p_node)
{
    ...
    newlist->pHead->pNext = nullptr;
    newlist->pTail->pPrev = nullptr;
    return newlist;
}
...
void removeBefore(d_List *&L, int val)
{
    if (L->pHead == nullptr || L->pHead->pNext == nullptr || L->pHead->key == val)
        return;
    d_NODE *cur = L->pTail;
    while (cur != nullptr)
    {   
...
void removeAfter(d_List *&L, int val)
{
    if (L->pHead == nullptr || L->pTail->key == val || L->pHead->pNext == nullptr)
        return;
    d_NODE *cur = L->pTail->pPrev;
    while (cur != nullptr)
    {   
        if (cur->key == val)
        {
...
void removePos(d_List *&L, int data, int pos)
{
    if (pos < 0 || L->pHead == nullptr)
        return;
    if (pos == 0 && L->pHead->pNext == nullptr)
    {
        d_NODE *temp = L->pHead;
        L->pHead = nullptr;
        L->pTail = nullptr;
...
{   
    if (L->pHead == nullptr)
    {
        d_List *rL = new d_List;
        rL->pHead = nullptr;
        rL->pTail = nullptr;
        return rL;
    }
    d_NODE* firstnode = createNode(L->pTail->key);
    d_List *rL = createList(firstnode);
    d_NODE* cur = L->pTail->pPrev;
    while (cur != nullptr)
    {   
        d_NODE* newnode = createNode(cur->key);
        rL->pTail->pNext = newnode;
        newnode->pPrev = rL->pTail;
        rL->pTail = newnode;
        cur = cur->pPrev;
    }
    rL->pTail->pNext = nullptr;
    return rL;
}
...
void removeDuplicate(d_List *&L)
{
    ...
    {   
        bool check = true;
        for (d_NODE *cur1 = cur; cur1 != nullptr ; cur1 = cur1->pPrev)
        {
            if (cur->pNext->key == cur1->key)
            {
                check = false;
                d_NODE* t = cur->pNext;
                if (t == L->pTail)
