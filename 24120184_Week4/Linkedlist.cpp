...
void removeBefore(List *&L, int val)
{
    ...
    NODE* cur = L->p_head;
    while (cur->p_next != nullptr && cur->p_next->p_next != nullptr)
    {
...
void removeDuplicate(List *&L)
{
    ...
    {   
        bool check = true;
        for (NODE* cur1 = L->p_head ; cur1 != cur->p_next ; cur1 = cur1->p_next)
        {   
            if (cur1->key == cur->p_next->key)
            {   
                check = false;
                NODE* t = cur->p_next;
                if (t == L->p_tail)
